...
export function ClassInfoCard({ classData, onManageStudents }: ClassInfoCardProps) {
  ...
  return (
    <Card className="rounded-2xl bg-white/60 backdrop-blur border border-black/5 shadow-sm overflow-hidden">
      {/* 渐变头部 */}
      <div className="bg-gradient-to-r from-[rgb(var(--accent-rgb)/0.12)] to-[rgb(var(--accent-rgb)/0.25)] px-5 pt-4 pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-white/70 flex items-center justify-center shadow-sm">
              <Users className="w-4.5 h-4.5 text-[color:var(--accent)]" />
            </div>
            <div>
              <p className="font-bold text-slate-800 leading-tight">{classData.name}</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                {classData.term && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-white/70 text-[color:var(--accent)] flex items-center gap-0.5">
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
              {classData.students.map(s => (
                <span key={s} className="text-xs px-2 py-1 rounded-lg bg-[rgb(var(--accent-rgb)/0.07)] text-slate-600 border border-[rgb(var(--accent-rgb)/0.12)]">
                  {s}
                </span>
              ))}
            </div>
